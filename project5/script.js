...
$(document).ready(function() {
  const tooltips = {
    ...
    'JQuery': 'A JS library that simplifies DOM manipulation.'
  };

 
  const $tooltip = $('<div class="tooltip"></div>').appendTo('body').hide();

  $('.coding li').on('mouseenter', function(e) {
    const text = tooltips[$(this).text().trim()];
    if (text) {
      $tooltip.text(text).fadeIn(200);
    }
  });

  $('.coding li').on('mousemove', function(e) {
    $tooltip.css({
      top: e.pageY + 10 + 'px',
      left: e.pageX + 10 + 'px'
    });
  ...
  $('.coding li').on('mouseleave', function() {
    $tooltip.fadeOut(100);
  });
});
